...
#define DBG_BaudRate     	            115200 //921600 //1843200 // 10800000 //460800 // 230400 //38400 //

/* Размер буфера диагностических сообщений */
#define DBG_UART_MAX_SIZE_BUFF       (500) 
...
void hal_debug_uart_init(void)
{
  ...
  */
  ...
  USART_InitStruct.BaudRate = DBG_BaudRate;
...
void USART2_IRQHandler(void)
{
	...
	{
		/* принимаем данные        */
		buf_cmd[index_wr_buf_cmd] =  LL_USART_ReceiveData8(USART2);
		/* нкремент индекса */ 
		index_wr_buf_cmd++;
		/* Проверка на переполнение  */
	...
	if ((LL_USART_IsActiveFlag_TXE(USART2)) & (LL_USART_IsEnabledIT_TXE(USART2)))		
	{
		...
		else
		{
			...
			LL_USART_TransmitData8( USART2, (uint16_t)(buf_dbg[index_rd_buf_mes]));      
			/* Увеличение индекса */
			index_rd_buf_mes++;
			/* Проверка на переполнение  */
